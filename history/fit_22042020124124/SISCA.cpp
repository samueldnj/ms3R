...
//      performance.
//  4. Logistic normal likelihood for compositional data.
                        ...
                        vector<int>   tInitModel_p )
{
  ...
  for( int p = 0; p < nP; p++ )
  {
    ...
    // Update recruitment vector
    if( t >= tInitModel_p(p) )
    {
      R_pt(p,t+1)  = reca_p(p)*SB_pt(p,t)/( 1. + recb_p(p)*SB_pt(p,t) );
      R_pt(p,t+1) *= exp( omegaR_p(p) );
    }
  }
...
Type objective_function<Type>::operator() ()
{
  ...
  // Recruitment devs
  array<Type> omegaR_pt(nP,nT);
  omegaR_pt.setZero();
  int recVecIdx = 0;
  for( int p = 0; p < nP; p++ )
    for(int t = firstRecDev_p(p); t < lastRecDev_p(p); t++)
    {
      omegaR_pt(p,t) = sigmaR * recDevs_vec(recVecIdx);
      recVecIdx++;
    }
  ...
  array<Type>   obsIdxNLL_pg(nP,nG);          // observation indices NLL
    ...
    for( int p = 0; p < nP; p++ )
    {
      if( t == tInitModel_p(p))
      {
        ...
        for( int a = 0; a < nA; a++)
        {
          if( initCode_p(p) == 1 )
          {
            N_apt(a,p,t) = Rinit_p(p) * initSurv_ap(a,p); // * initN_mult_ap(a,p);
            
            if( initMethod == "nums" )
              N_apt(a,p,t) *= initN_mult_ap(a,p);
          }

          if( initCode_p(p) == 0 )
            N_apt(a,p,t) = R0_p(p) * surv_ap(a,p);
        }
  ...
  for( int t = 0; t < nT; t++ )
  {
    ...
    for( int p = 0; p < nP; p++ )
      if( psi_pt(p,t) > 0 )
        psi_nlp -= dnorm( log(psi_t(p,t)), log(mPsi), sdPsi, true);
  }
