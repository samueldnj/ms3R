...
Type objective_function<Type>::operator() ()
{
  ...
  // Fishery removals
  PARAMETER_VECTOR(lnqF_g);             // Vector of catchabilities to scale effort to removals
  ...
  PARAMETER_VECTOR(sdlnqF_g);           // Fleet specific prior sd on removal catchability
  ...
  for( int p = 0; p < nP; p++ )
    ...
    {
      omegaR_pt(p,t) = -5 + 10 / (1 + exp(-recDevs_pt(p,t-1)));
    }
  ...
  array<Type>   F_apgt(nA,nP,nG,nT);        // Fleet fishing mortality at age
  ...
  for( int t = 0; t < nT; t++ )
  {
    ...
    for( int p = 0; p < nP; p++ )
    {
      if( t == tInitModel_p(p))
      {
        ...
        if( densityDepM == 1 )
          M_apt.col(t).col(p).segment(juveMage,nA - juveMage) = M_p(p) + exp( -m1_p(p) * B_pt(p,t)/totB0_p(p) );
    ...
    for( int cIdx = 0; cIdx < nG; cIdx ++ )
    {
      ...
      if( catSeriesType_g(gIdx) == 1)
        totC_pgt.col(t).col(gIdx) *= exp(lnqF_g(gIdx));
      ...
      for( int p = 0; p < nP; p++)
      {
        if( t >= tInitModel_p(p) )
        {
          for(int a = 0; a < nA; a ++ )
          {
            ...
            catAge_apgt(a,p,gIdx,t) = uAge_apgt(a,p,gIdx,t) * totC_pgt(p,gIdx,t) / wtAge_apg(a,p,gIdx);
    ...
    for( int p = 0; p < nP; p++ )
    {
      if( t >= tInitModel_p(p) )
      {
        for(int a = 0; a < nA; a++ )
        {
          ...
          endN_apt(a,p,t) = tmpN_ap(a,p) * exp( - lastFrac * M_apt(a,p,t)); 
          ...
          for( int g = 0; g < nG; g++ )
            if( fleetType_g(g) == 2 | fleetType_g(g) == 3)
            {
              ...
              endN_apt(a,p,t) += pondC_apgt(a,p,g,t) * exp( -postPondM_g(g) - fleetFrac * M_apt(a,p,t));
            }
          ...
          Z_apt(a,p,t) = -log(endN_apt(a,p,t)/N_apt(a,p,t));
          ...
          if( a == 0 )
          {
            ...
            // BH recruitment
            if( avgRcode_p(p) == 0 )
              N_apt(a,p,t+1) =  bhR_pt(p,t+1) * exp(sigmaR * omegaR_pt(p,t+1));              
      ...
      if( densityDepM == 1 & t >= tInitModel_p(p) )
      {
        ...
        if( t < nT - 1 )
          M_apt.col(t+1).col(p).segment(juveMage,nA-juveMage) *= exp(omegaM_pt(p,t));
      }
      ...
      if( calcIndex_g(gIdx) == 1)
      {
        ...
        for( int t = tInitModel_p(p); t < nT; t++)
        {
          ...
          if( I_pgt(p,gIdx,t) >= 0.0 )
          {
            ...
            {
              idxState = (vulnB_apgt.col(t).col(gIdx).col(p)*mat_a).sum();
            }
    ...
    for( int t = tInitModel_p(p); t < nT; t++ )
    {
      ...
      if( combI_pt(p,t) >= 0.0 )
      {
        ...
        if( combI_pt(p,t) > 0 )
        {
          ...
          // Now apply probability of positive index
          // to expected state variable
          Type tmpB_t = (vulnB_apgt.col(t).col(4).col(p) * mat_a).sum();
          expIdx = qComb_pt(p,t) * tmpB_t * probPosCombIdx_pt(p,t);
          // Calculate residual
  ...
  if( densityDepM == 1 )
  {
    mort_nlp -= dnorm( log( M0_p ), log(initMPrior(0)), initMPrior(1),true).sum();
    mort_nlp -= dnorm( lnM, Type(log(0.2)), Type(0.1));
  ...
  {
    if(catSeriesType_g(gIdx) == 1)
      lnqF_nlp -= dnorm(lnqF_nlp,mlnqF_g(gIdx),sdlnqF_g(gIdx),true);
