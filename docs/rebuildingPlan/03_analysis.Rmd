...
with a strongly informative prior, estimates of 2019 biomass did not 
...
Operating model data likelihood values were split into two distinct 
groups, defined by the time-varying natural mortality hypothesis
(Table \@ref(tab:omGridTable)). The historical OM scenario with the 
minimum negative log likelihood (NLL) was the **a1Mmean_sr0.8_diffM** 
model, indicating the lowest residual standard error. Reduced residual 
error was driven primarily by the **diffM** hypothesis, which allowed 
the Lou 2+ mortality to drop at the end of the historical period, causing 
Lou spawning biomass to increase over that period, matching the Lou spawn 
indices. As a result, the **diffM** scenarios had the lowest negative log 
likelihood (NLL) function values, with differences 
($\Delta(-\log\mathcal{L})$) from **a1Mmean_sr0.8_diffM** scenario 
between 0 and 10.22 NLL units. In contrast, the **identM** scenario
$\Delta(-\log\mathcal{L})$ values ranged between 35.03 and 41.50 NLL 
units.
...
seine-roe **SR_HS30-100_HR.1** fishery.
...
mortality, despite higher unfished biomass $B_0$, so true OM biomass 
rarely exceeded the lower control point, requiring assessment errors to 
open fishing in most years. Finally, scenarios in the middle of the 2019 
depletion range (e.g., **a1M1.25**) had more room to grow and had higher 
average recruitments for a given biomass, but biomass was unable to 
increase fast enough to achieve the maximum harvest rates, largely because 
of the initial conditions. 

Under the most optimistic operating model scenario
(**a1Mmean_sr0.8_diffM_rw5_loPondM**), there was very little 
separation between the five rebuilding procedures according
to the biomass metrics (Table \@ref(tab:bestOMperfTable)).
...
For example, there were only 1-2 more years with SOK openings, on 
average, with an average of 4.1 - 4.8 licenses activated in an 
open year.
