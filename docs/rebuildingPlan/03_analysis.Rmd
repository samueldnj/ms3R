...
<!-- For example, when a sub-area biomass forecast was below the lower control point 
($B_{p,t+1}  < 0.3 \overline{B}_{0,p}$) the target harvest rate was set to zero, 
and when the stock was at or above the upper control point 
($B_{p,t+1} \geq \overline{B}_{0,p}$) the target harvest rate was set to 10\%. The 
harvest rate was linearly interpolated between 0\% and 10\% when biomass 
was between these two control points. --> 
...
performance metrics for sub-areas are provided in an appendix
**(TO DO: Reference Appendix tables here)**
