...
# Save Bridge Analysis Table

bridgePerfStats <- lapply( X = batchBlobs, FUN = .simPerfStats, inclAgg=TRUE, exclNoTAC=FALSE )

bridgeTable <- do.call(rbind, bridgePerfStats)  %>%
               select(scenario, mp, stock, 
                      pBtGt.3B0, pBtGt.6B0, catchAAV, avgCatch_t, pCtGt650t) %>%
               arrange(scenario, -1*pBtGt.3B0,-1*pBtGt.6B0) %>%
               subset(stock=='aggregate')

bridgeTable$pCtLt650t <- 1- bridgeTable$pCtGt650t
bridgeTable <- dplyr::select(bridgeTable, pBtGt.3B0, pBtGt.6B0, catchAAV, avgCatch_t, pCtLt650t)


write.csv(bridgeTable,  file.path(batchDir,'batchReports','bridgeTable.csv'), row.names=F)
