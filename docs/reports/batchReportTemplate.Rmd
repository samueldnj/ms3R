...
batchPerfStats <- lapply( X = batchBlobs, FUN = .simPerfStats, inclAgg=TRUE )
statTable      <- do.call(rbind, batchPerfStats)  %>%
                  select(scenario, mp, stock, pBtGt.3B0, avgCatch_t:pondAAV) %>%
                  arrange(scenario, -1*pBtGt.3B0, avgCatch_t, avgPonded_t)
