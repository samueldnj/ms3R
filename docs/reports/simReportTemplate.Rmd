...
plotTulipBt( obj = blob, var='SB_ispt', dep = TRUE, Ct = TRUE )
```

```{r, plotTulipBtCtProj, echo = FALSE, message = FALSE, fig.cap = SBsptCaption }
plotTulipBt( obj = blob, var='SB_ispt', dep = TRUE, Ct = FALSE, tMin=60 )
```


## Catch and Fishing mortality

```{r, plotTulipCt, echo = FALSE, message = FALSE, fig.cap = CtTulipCaption }
plotTulipCt( obj = blob, tMin=60, plotFleets=TRUE )
```

```{r, plotTulipF, echo = FALSE, message = FALSE, fig.cap = EffTulipCaption }
plotTulipHR( obj = blob )

# TO DO: change this to plot HCR
...
```{r, phaseTable, echo = FALSE, eval = pT > 15 }

statTable <- .simPerfStats( obj = blob, inclAgg=TRUE, exclNoTAC=TRUE ) %>% 
            dplyr::select(stock, pBtGt.3B0, nYrsSOK:catchAAV)
...
```{r, bridgingTable, echo = FALSE, eval = pT > 15 }

bridgeTable <- .simPerfStats( obj = blob, inclAgg=TRUE, exclNoTAC=FALSE ) %>%
              dplyr::select(stock, pBtGt.3B0, pBtGt.6B0, catchAAV, avgCatch_t, pCtGt650t)

# statTable <- dplyr::select('stock', )

kable(  bridgeTable , escape = FALSE, 
        caption = "Performance statistics for Bridging Analysis.", booktabs = T,
        align = rep("c",ncol(bridgeTable))) %>%
  kable_styling( latex_options = c("striped", "scale_down","hold_position"),
                 bootstrap_options = c("striped", "hover"))
...
if(   !(blob$ctlList$ctl$omni | blob$ctlList$ctl$perfConF)  & 
    ( !blob$ctlList$mp$assess$method %in% c("ItCt","idxBased","perfectInfo" ) ) )
{
...
```{r diagCondition, echo = FALSE, message = FALSE}
diagCondition(  repObj  = blob$ctlList$opMod$histRpt,
                ms3Obj  = blob,
                iRep    = 1 )
```
