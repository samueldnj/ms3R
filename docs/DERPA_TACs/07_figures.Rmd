...
different AMs under Rich and Poor data scenarios. Panels are 
gridded by species (columns) and stocks (rows), with biomass 
...
for over all replicates for different AMs, with AMs under the
same scenario joined by a solid line (Rich) or dashed line 
(Poor)."

batchLossSensSummaryCap <- "Regressions showing the average 
sensitivity of cumulative catch loss to the prior standard
deviations under each AM (colours, line types) for the 
...
difference between median cumulative loss for an AM and the mean
...
```{r omniBtCtTulips, echo = FALSE, warning = FALSE, fig.cap = omniBtCtTulipsCap, fig.width = 9, out.width = "9in"  }
figPath <- file.path("./data/omniRun_TulipBt.pdf")
knitr::include_graphics(figPath)
```
...
figPath <- file.path("./data/omniEffortTulip.pdf")
knitr::include_graphics(figPath)
```
...
```{r batchLoss-SensitivityRuns, echo = FALSE, warning = FALSE, fig.cap = batchLossSensSummaryCap, fig.width = 8, out.width = "8in"  }
...
par( mfrow = c(3,2), oma = c(3,3,2,1), mar = c(2,2,1,1) )
leg <- TRUE
for( sensIdx in 1:3 )
{
...
leg <- FALSE
  ...
  if( sensIdx == 1)
    mtext( side = 3, text = "Poor", font = 2)

  rmtext( line = .05, txt = xlab[sensIdx], font = 2, outer = TRUE)
}
