...
N_{a,x,s,p,t} = \left \{ \begin{array} {ll}
                          ...
                          N_{a-1,x,s,p,t-1} \cdot e^{-Z_{a-1,x,s,p,t-1}} + N_{a,x,s,p,t-1}\cdot e^{-Z_{a,x,s,p,t-1}} & a = A_s,
                          \end{array}
                \right.
\end{equation*}
where $R_t$ is age-1 recruitment in year $t$, $Z_{a,x,s,p,t}$
is the instantaneous total mortality rate, and $A_s$ is the plus
...
N_{a,x,s,p,1956} = \left \{ \begin{array} {ll}
                          ...
                          N_{a-1,x,s,p,1956} \cdot \frac{e^{-M_{x,s,p}}}{1 - e^{-M_{x,s,p}}} & a = A,
                          \end{array}
                \right.
\end{equation*}
...
Catch-at-age was then found via the Baranov catch equation
\begin{equation*}
...
DER complex data [Johnson and Cox, in prep]. To improve convergence 
in the simulations, we multipled all estimates by $0.3$ to improve
observation model precision (multiplier found by trial and error).
...
tauObs_spf <- round(tauObs_spf * 0.3,3)
...
\clearpage
# Hierarchical model performance under relaxed shrinkage prior SDs

```{r loadSimForRetroPlot, echo = FALSE, include = FALSE}
# .loadSim( 6, folder = "sensRuns_hierSD")

sensRunTulipCap <- "Spawning biomass depletion and relative catch 
simulation envelopes for all nine DER complex management units when 
assessed by the hierarchical multi-stock asssessment model under the 
Poor OM data quality scenario, when hierarchical shrinkage prior SDs
were $\\sigma_{U_{MSY}} = \\tau_{q} = 0.5$. Median biomass is shown by 
the thick black line, with the grey region showing the central 95\\% of the 
...
sensRunRetroCap <- "Operating model spawning stock
...
from the first simulation replicate in the Poor data quality scenario
under the hierarchical multi-stock assessment model when hierarchical 
shrinkage prior SDs were $\\sigma_{U_{MSY}} = \\tau_{q} = 0.5$. Catch 
bars show realised catch in grey for the whole simulation period, and 
unfilled bars in the projection period show the difference between MP 
set TACs and realised catch. Coloured circles on the right hand vertical axis 
...
\blandscape
```{r figC1-tulipBtCtHMSPoorHierSD, echo = FALSE, warning = FALSE, fig.cap = sensRunTulipCap, fig.width = 9, out.width = "9in" }
...
figPath <- file.path("./data/figC1-tulipBtCtHMSPoorHierSD.pdf")
...
```{r figC2-retroBioSingleRepHMSPoorHierSD, echo = FALSE, warning = FALSE, fig.cap = sensRunRetroCap, fig.width = 9, out.width = "9in" }
# plotRetroSBagg(  obj = blob, iRep = 1, plotStamp = FALSE )
figPath <- file.path("./data/figC2-retroBioSingleRepHMSPoorHierSD.pdf")
knitr::include_graphics(figPath)
