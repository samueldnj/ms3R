...
flatfish fishery. When available data quantity was moderate or low 
...
high data quantity scenarios, data-pooling methods outperformed hierarchical 
models, but the latter still outperformed single-stock assessment methods. 
This suggests that hierarchical assessment methods could be a better 
approach to making catch limit decisions than conventional single-species 
methods under typical fisheries data quality conditions, such as short or 
noisy time series of fishery-independent observations.
...
exhibited above. However, in data-limited contexts where biological data 
are of low sample size, there would be even greater reliance on 
strong *a-priori* assumptions for additional parameters in models of 
higher complexity, which we predict would lead to similar results.
...
while improved performance relative to single-stock models under lower 
...
fundamentally different mechanism. This difference can be explained
