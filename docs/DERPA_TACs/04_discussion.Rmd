...
rates would likely lead to higher instances of choke effects, given the 
ignorance of technical interactions, but would probably not affect the 
rankings since the estimated optimal harvest for hierarchical models 
had compensatory biases in biomass and harvest rates, which would hold
for the single-species harvest rates as well. On the other hand, 
single-species and pooled models lacked the compensatory biases
in biomass and harvest rates, and pooled models had issues achieving
target TACs stemming from the catch allocation model. If ranking by
...
sustainable. While it is possible to *a priori* define commercial 
catchabilities so that multi-species optima would produce critically 
overfished stocks, that was not the case for the simulated DER complex. 
