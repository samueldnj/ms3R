...
a technically interacting group of flatfishes managed over the
BC coast (Figure 1). Although there are several right-eyed flounders in 
these waters, we focus on the three species, indexed by $s$, Dover 
...
\end{equation}
where $MSY_{s,p}$ (optimal single-species yield) and
$U_{MSY,s,p}$ (optimal single-species harvest rate) are the 
leading biological model parameters, 
$B_{MSY,s,p} = MSY_{s,p} / U_{MSY,s,p}$ is the biomass at which $MSY_{s,p}$ 
is acheived under the optimal harvest rate, $m_{s,p}$ is the 
Pella-Tomlinson parameter controlling skew in the biomass/yield 
relationship (derived from operating model yield curves), 
and $\zeta_{s,p,t}$ are annual process error deviations. 
...
catch rates are biased indices of abundnace, but both fishery independent 
trawl survey indices had log-normally distributed observation error 
deviations with precision based on the conditioning assessment 
