...
since before 1956 (Figure 1). At that time, Dover sole was only 
...
is concentrated  for those species, with fishing 
activity in QCS and WCVI areas more variable 
over time (Figure 1, rows 2 and 3).

- Stock status, reference points,
data series observation error CVs as estimated
by hierSCAL (table)
...
Our closed loop simulations applied the 
management procedure iteratively in the 
following 5 steps over the $T$ 
...
If we do the survey CV scenarios, then we should restrict
the MP options to 10. (5 AMs, and 2 Fs)

Synoptic Survey observation error standard deviation scenarios:
2. 0.5 times current SD: $\tau_{s,p,f,proj} = 0.5 \cdot \tau_{s,p,f}$;
3. Current $\tau_{s,p,f}$ value from conditioning;
4. 1.5 times current SD: $\tau_{s,p,f,proj} = 2 \cdot \tau_{s,p,f}$;
...
of 1.5 times the OM value in our scenarios because the average
OM observation error SD across all stocks was around $0.5$ for 
the synoptic survey, which is already very uninformative, 
and in preliminary simulations we found that the 
convergence rate of single-stock and hierarchical 
multi-stock assessment model configurations 
became much lower when.



### Management procedures

We tested ten MPs, which were factorial combinations
of the five assessment model configurations outlined above,
and the two target harvest rates, corresponding
to optimal yield when considering a single-species or
the multi-species complex in each area. We chose to restrict
