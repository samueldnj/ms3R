...
commercial catchability, as well as the
relative catchabilities between all species
in the complex [@punt2011calculating; Johnson and 
Cox in prep]. We defined multi-species MSY for stock-area $p$ as
\begin{equation}
...
model because of (i) its simplicity and (ii) it is analogous
to fully realising a single-species TACs in single-species closed
loop simulation. We considered including an effort dynamics 
...
### Assessment models, data generation
...
#### Target harvest rates, total allowable catch

Simulated harvest controls applied 
...
1. **HMAS** Historical CPUE, Modern CPUE, Assemblage 
survey, Synoptic survey;
2. **MAS** Modern CPUE, Assemblage survey, Synoptic survey;
3. **MS** Modern CPUE, Synoptic survey;
4. **AS** Assemblage survey, Synoptic survey.
...
the **AS** scenario was Synoptic only for 6 stocks.
...
manager. For each stochastic trajectory, the 
relative and absolute loss of catch and biomass
was calculated as [@walters1998evaluation]:
\begin{align}
L_{abs} &= \sum_{t = T_1}^{T_2} | X_{t,sim} - X_{t,omni} |, \\
L_{rel} &= \sum_{t = T_1}^{T_2} | X_{t,sim} - X_{t,omni}|/X_{t,omni}, 
\end{align}
where the $X_{t,\cdot}$ values are either catch or spawning
biomass series from stochastic simulations ($sim$) 
or the omniscient manager simulation ($omni$)
simulation. When repeated over all random seed values, 
