...
lightly exploited, with full commercial level exploitation
beginning in the 1970s. English and rock soles and 
...
and the OM conditioning assessment.
...
We measured our MP performance against the a simulated
...
to using standard MSE metrics, such as depletion and catch
levels, because several stocks are in a healthy state. Being 
in a healthy state means that the stock behaves more like
...
TRAWL FOR INPUT/POSSIBLE CITATION HERE**). **Tuning criteria?**

We used a cubic spline of effort in each area to reduce
the number of free parameters in the optimisation. For each
area, 11 knot points were distributed across the full 40 year
projection, making them spaced by 4 years. Effort splines
were constrained to be between 0 and 50 units, by simply
...
\end{align}
where the $X_{t,MP}$ values are either catch or 
