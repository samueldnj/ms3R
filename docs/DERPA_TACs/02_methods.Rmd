...
we focus on the three species, indexed by $s$, Dover sole ($s = 1$), 
English sole ($s = 2$), and southern Rock sole ($s = 3$), which we hereafter 
refer to as Rock sole. Taken together, these species comprise a multi-stock 
complex (DER complex), managed as part of the BC multi-species groundfish 
fishery.

The DER complex is managed in three spatially distinct stock
areas, indexed by $p$ (Figure 1) [@PRIFMP2015]. From north to south, the 
...
from the southern tip of Haida Gwaii to the northern tip of Vancouver Island. 
Finally, the third area - West Coast of Vancouver Island (WCVI, $p = 3$) - 
extends from the northern tip of Vancouver Island south to Juan de Fuca Strait.

Each species/area combination had commercial trawl catch for the entire 
history (1956 - 2016), two commercial catch rate series, and at least one survey 
biomass index time series for each species (Figure 2), for a total of
...
area (total nine stocks) forward in time for $32$ years (two Dover sole 
generations), with annual simulated assessments, harvest decisions, and 
catch removed from the population from 2017 - 2048. The following four 
steps summarise the closed-loop simulation procedure for each 
projection year $t$:
...
spawning biomass for the upcoming year $\hat{B}_{s,p,t+1}$ and
an optimal harvest rate $\hat{U}_{MSY,SS,s,p}$ (eq \ref{eq:amBio});
4. Scale the estimated optimal harvest rate to an estimated multi-species 
optimal harvest rate, and use it with the biomass forecast generate a total 
allowable catch $TAC_{s,p,t+1}$, allocated among species/areas if using 
a pooled method (eqs \ref{eq:startHR} - \ref{eq:endHR});
...
explicit effort dynamics model because the former captured the choke 
effects present in the real fishery system, as well as reflects that
the BC groundfish fishery is an output control fishery, with no explicit
...
\end{equation}
where $MSY_{s,p}$ (optimal single-species yield) and $U_{MSY,s,p}$ (optimal 
single-species harvest rate) are the leading biological model 
parameters, $B_{MSY} = MSY / U_{MSY}$ is the biomass at which $MSY$ is
acheived under the optimal harvest rate, $m_{s,p}$ is the Pella-Tomlinson 
parameter controlling skew in the biomass/yield relationship (derived 
from operating model yield curve), and $\zeta_{s,p,t}$ are annual process 
error deviations. 
...
2. Species Pooling (3 management units, independent);
3. Spatial Pooling (3 species, independent);
4. Single-stock (9 management units, independent);
5. Hierarchical Multi-species (9 management units, sharing information);
...
Prior distributions on optimal yield $MSY_{s,p}$, optimal 
harvest rate $U_{MSY,s,p}$, catchability $q_{s,p,f}$, and
process error deviations $\zeta_{s,p,t}$ were defined for each 
assessment model, with unbiased prior means based on the 
true operating model values (Supplement S.1). While unbiased prior distributions 
are not possible to define in practice, our research question focused 
on the effect of the five AM structures on management performance when 
setting TACs based on AM estimates of biomass and productivity. While priors
certainly affect those estimates, there is considerable variation in how an 
analyst might choose prior mean values for biological parameters, and those
choices become more uncertain as data are reduced. For example, an analyst 
might choose a prior mean of average catch over an agreed historical period 
as the $MSY$ prior mean, and choose either a conservatively low value based
off a review of stock assessments for similar species, or if it is available,
the ratio of catch to an index of biomass in the same historical period to 
inform a prior mean $U_{MSY}$ value. In cases where time-series of data are 
short, there may be no appropriate period for formulating such priors, and 
when data are noisy (e.g., commercial CPUE) it may have insufficient statistical 
power to draw model estimates away from biased mean values, especially when priors
may be informative in the presence of data limitations. We based our unbiased 
priors off the OM to keep priors as similar as possible between AMs, thereby 
avoiding variation in performance caused by different prior choices among AMs.


### Data generation for assessment models

Time series of catch and biomass indices were simulated in the historical 
and projection periods for fitting assessment models. While it is more
usual to use the actual historical data in most closed loop simulations, we 
decided that given that pooled data must be simulated, simulating all data 
series removed any effect of the difference between simulated and real data
...
index were also simulated in the projection period. Both commercial CPUE 
series were simulated without additional observation error deviations, as 
catch rates are unlikely to be unbiased indices of abundnace, but both 
fishery independent trawl survey indices had log-normally distributed 
observation error deviations with precision based on the conditioning 
assessment (Table A.1, Appendix A).
...
\begin{align} 
\bar{I}^{pooled}_{s,f,t}  & = \sum_{p} \mathbb{1}{\mathcal{I}}(I_{s,p,f,t} > 0) \cdot q_{s,p,f} \cdot B_{s,p,f,t} \\
\bar{I}^{pooled}_{p,f,t}  & = \sum_{s} \mathbb{1}{\mathcal{I}}(I_{s,p,f,t} > 0) \cdot q_{s,p,f} \cdot B_{s,p,f,t} \\
\bar{I}^{pooled}_{f,t}    & = \sum_{s,p} \mathbb{1}{\mathcal{I}}(I_{s,p,f,t} > 0) \cdot q_{s,p,f} \cdot B_{s,p,f,t}
\end{align}
...
error), and $\mathbb{1}{\mathcal{I}}(I_{s,p,f,t} > 0)$ is the indicator 
...
\end{equation}
where is the target harvest rate for species $s$ in stock-area $p$, 
and $\hat{B}_{s,p,t+1}$ is the year $t+1$ biomass forecast from the 
assessment model. The target harvest rate was 
\begin{equation}
U_{s,p} = \frac{U_{MSY,MS,s,p}}{U_{MSY,SS,s,p}} \cdot \hat{U}_{MSY,s,p}
\end{equation}
...
taken from the operating model (Table 1), and $\hat{U}_{MSY,s,p}$ is 
the assesment model estimate of the single-species optimal harvest 
rate, thereby including assessment error while attempting to maximise 
complex yield and avoiding including technical interactions and effort 
...
reduces overfishing as a result of large positive assessment errors, 
...
totally pooled rates defined analogously. Assessment model estimates of 
...
TAC'_{s,p,t+1} = \frac{\bar{I}_{s,p,t^*}}{\sum_{s'} \bar{I}_{s',p,t^*}} TAC_{p,t+1},
\end{equation}
...
performed by a real-life analyst. Any species/stock combinations 
that could not reach 100 replicates meeting the 95\% convergence criterion 
for all AMs were excluded from the results. The operating model was run 
...
matched their expected values, i.e., for the two fishery
independent surveys ($f = 3,4$), the species/area specific
...
year, respectively. Recruitments are simulated as in appendix A. Error is added 
to survey biomass indices for pooled data independently of the error added to individual 
indices, i.e.,
\begin{align}
...
1. Data-**Rich**: Historical CPUE (1956 - 1996), Modern CPUE (1996 onwards), 
...
the **Mod** and **Poor** scenarios, respectively. While it may be unusual to
consider the case with only fishery-independent data as "data-poor", 
we are defining this based on time-series length relative to the stock 
history, and observation series precision. The Assemblage survey was 
highly variable, with a biennial frequency on a design that could
be considered a fixed station survey, and only surveys one area
out of the three. Similarly, the Synoptic survey time series is quite short, 
with only 7 or 8 observations in each area, covering only the most recent 
23\% of the historical period, and it is not optimally designed to survey any 
one groundfish species. The production model AMs will struggle to estimate
biomass and productivity using only observations from a short time period at 
the end of a stock's one-way trip history that misses most of the original 
depletion signal. 

We did not include a scenario where only fishery CPUE is used for management 
purposes, as this is not a realistic case for the real BC groundfish fishery, 
and we preferred not to introduce variability between scenarios by defining 
a new TAC allocation method, which would have been necessary in the absence 
of the Synoptic survey index.
...
transient dynamics at the beginning of the projection, or a lack of 
consequences for overfishing at the end of the projection.
...
omniscient manager simulation ($omni$) simulation, and their difference
between the absolute value symbols is defined simply as catch loss. When 
repeated over all random seed values, the loss functions generated a 
distribution of cumulative absolute catch loss, which were then used to 
determine relative performance of each assessment model under the three data 
scenarios. Cumulative absolute catch loss was calculated for the ten year 
period $T_1 = 2028$ to $T_2 = 2037$, chosen in the middle of the projection 
period because dynamics in the earlier period were dominated by the smoothers 
on effort and catch for the omniscient manager and TACs, respectively.

A paired analysis was used to determine rank AM configurations across species, 
stocks and replicates. Within each replicate, each AM's cumulative absolute catch 
loss determined the relative rank of each AM under a species/stock/OM scenario 
combination, where lower loss ranked higher. Any replicates with less than 95\% 
convergence rates for any AM on any management units were excluded from the 
aggregate rankings, and any species/stock combinations that failed to reach 
100 replicates meeting the convergence criteria for all AMs were excluded as 
well, to reduce variability in ranking distributions caused by varying sample 
sizes and random seeds. Rankings were then pooled across remaining species and 
stocks within an OM/AM combination, from which the modal rank and average rank
were calculated.
...
both 80\% and 40\% of single-species $B_{MSY,SS,s,p}$. The threshold of 
80\% of $B_{MSY,SS}$ is generally considered to be the level where a 
fish stock transitions from an optimally fished state to an overfished 
state [@hilborn2018measuring], while 40\% of $B_{MSY}$ is commonly 
suggested as a limit reference point for Canadian fisheries, below which 
a stock may be considered critically overfished as recruitment may become 
impaired and rebuilding may be required [@DFO2006A-Harvest-Strat]. For 
both biomass levels, the probability was calculated as
\[
P(B_{s,p,t} < \lambda B_{MSY,SS,s,p} ) = \frac{1}{3200} \sum_{i = 1}^{100}\sum_{t = 2016}^{2047} \mathbb{1}(B_{i,s,p,t} < \lambda B_{MSY,SS,s,p})
\] 
where $i,s,p,t$ are replicates, species, stocks and time steps, 
respectively, $\lambda = 0.4,0.8$, and $\mathbb{1}$ is the indicator
function that takes value $1$ when its argument is true, and zero
otherwise.

Biomass loss was also calculated, but ultimately it was not reported. 
Inferences about assessment method performance in preliminary 
simulations were not qualitatively different between catch and 
biomass loss.
...
increases in fishery independent survey precision were tested, where 
