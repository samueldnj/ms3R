...
to fishing mortality (Table 1). The relationship equation 
...
simulated without additional observation error deviations, as catch rates 
are unlikely to be unbiased indices of abundnace, but both fishery 
independent trawl survey indices had log-normally distributed observation 
errors with precision based on the conditioning assessment 
...
dynamics in the earlier period were dominated by the smoothers on effort 
and catch for the omniscient manager and TACs, respectively.
...
replicate, each AM's cumulative absolute catch loss was used to 
determine the relative rank of each assessment model under a 
species/stock/OM scenario combination, where lower loss ranked higher.
Any replicates with less than 95\% convergence rates for any AM on any 
management units were excluded from the aggregate rankings, and any 
species/stock combinations that failed to reach 100 replicates meeting 
the convergence criteria for all AMs were excluded as well, to reduce
variability in ranking distributions caused by varying sample sizes. 
Rankings were then pooled across remaining species and stocks within
an OM/AM combination.


#### Biomass risk

Biomass risk was measured by the probability of stock biomass being below
both 80\% and 40\% of single-species $B_{MSY,SS,s,p}$. As outlined above, 
40\% of $B_{MSY}$ is commonly suggested as a limit reference point for 
Canadian fisheries, below which a stock may be considered critically 
overfished as recruitment may become impaired and rebuilding may be 
required [@DFO2006A-Harvest-Strat], while 80\% of $B_{MSY,SS}$ is
generally considered where a fish stock transitions from an optimally 
fished state to an overfished state. For both biomass levels, the
probability was calculated as
\[
P(B_{s,p,t} < 0.4 B_{MSY,SS,s,p} ) = \frac{1}{3200} \sum_{i = 1}^{100}\sum_{t = 2016}^{2047} \mathbb{1}(B_{i,s,p,t} < \lambda B_{MSY,SS,s,p})
\] 
