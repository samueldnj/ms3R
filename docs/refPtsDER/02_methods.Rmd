...
associated biomasses, catches, resource rents, and harvest rates (denoted
by adding an asterisk to the variable denoting steady-state optima) were 
estimated as the median of each quantity across the distribution of effort 
time series under each objective function and projection scenario. Median 
values were taken over the time period 2040 - 2060, which allowed 
enough time for the effort to stabilise after any initial transient 
period, but also avoided end effects caused by a finite projection
time horizon. Dynamic optima were then compared to the steady-state optima 
found via the numerical yield curve analysis outlined above, and the 
tails of catch and resource rent distributions were compared between
correlation scenarios to understand the effect of correlation on fishery 
risk.
...
the tails than the mean or median outcome, we measure risk as the
...
of risk commonly used in economic analyses [REF].
