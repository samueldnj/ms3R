...
bio-economic simulation for the fishery, multi-species $MSY$ and $MEY$ 
reference points and their derivation via yield-per-recruit analysis and 
stochastic optimisation, and, finally, a closed loop simulation approach 
to comparing management performance among the harvest strategies in the 
...
model representing the fishing costs and revenues resulting from 
...
barriers flatfish migration among areas, along with evidence of limited 
...
multi-stock (area) population dynamics model [@johnson2021hierarchical]. 
...
spawning biomass, unfished biomass $B_0$ producing unfished recruitment
...
proportion of individual fish of each age available to the 
...
curve relating the unit price to the quantity landed, and $c_{p}$ 
...
of fuel to catch one kilotonne of groundfish catch in 2009
...
for economic yield. Fixed costs were assumed to be constant, and thus
ignored, being generally difficult to assign to individual species or
multi-species complexes because most vessels fish for different groundfish 
species at different times of year, and sometimes with different gear types.
...
biomass into the equation \ref{eq:resRent} under 2016 inverse demand 
curves.
...
@hoshino2018estimating]. If demand has infinite own-price elasticity (i.e., 
a constant price with respect to landings), as is commonly assumed, then 
the surface may be optimised individually for each area. If, instead, a 
finite own-price elasticity is assumed (Appendix A), then the unit price for 
each species changes with the quantity landed, and spatial heterogeneity in 
species compositions implies spatial heterogeneity in rents, beyond the 
costs of fishing. The resulting relationship between rents and effort 
defines a surface over three dimensional effort space (one for each 
area), and optimal effort and $MEY$ reference points are found via 
...
fishing effort, assuming 2016 inverse 
...
the series of of NPV maximising efforts for each area, $MEY$ reference points 
...
\end{equation*}
to estimate $MSY_{MS}$. The second objective function maximises the net 
present value (NPV) of the DER complex resource rent over the same 
time horizon, i.e.,
\begin{equation*}
\pi^{obj} = \sum_{t = 2017}^{2096} (1 + d)^{-(t - 2016)} \sum_{p} \left[  \left( \sum_s v_{s,t} C_{s,p,t} \right) - c_{p} E_{p,t} \right] 
\end{equation*}
to estimate $MEY$, where $d$ is the annual discount rate (Table 2).

Simulations are repeated over 100 trials, where each trial involves
a unique realization of a stochastic future recruitment via
\begin{align*}
...
\end{equation}
is the expected Beverton Holt recruitment at time $t$ with life-history
parameters taken from Table 1, $B_{s,p,t}$ the spawning biomass of species
$s$ in area $p$ at time $t$,$\epsilon_{s,p,t} \sim N(0,1)$ is the standardised 
...
over over the 2040 - 2060 period under the yield ($C^{obj}$) and NPV 
...
While total effort is unconstrained in the simulations, inter-annual 
changes in coastwide effort were constrained to +/-20\% per year
to reflect the potential for adding/subtracting new licenses in the 
fishery, as well as the lag time required for a capital investment (e.g., 
in new and larger vessels). While the effort is eventually unbounded, the 
optimisation method treats the fishery as a single firm, rather than an 
open access regime, so it is possible that fishery profits will not be 
dissipated. The optimisation was the same as the omniscient manager 
simulation in @johnson2021hierarchical.

## Harvest strategy performance

While optimal fishing efforts have theoretical value, it is difficult 
to control total fishing mortality via input fishing effort alone,
...
The tradeoffs of repeated application of harvest strategies based 
on $MSY_{SS}$, $MSY_{MS}$ and $MEY$ reference points to the DER complex
are estimated via closed loop feedback simulations over an 80-year time
...
from the stochastic optimisation yield ($MSY_{MS}^*$) or NPV ($MEY^*$). 
...
target harvest rates). Conservation performance is determined based on the 
distribution of spawning biomass compared to single-species $B_{MSY,SS}$ 
over the period 2040 - 2060. Economic performance is measured via the NPV 
of the DER complex fishery over the whole 80-year time horizon. Risk was 
...
Three scenarios test alternative demand assumptions, specifically a flat 
demand relationship, simulated by refitting demand curves with an infinite 
price elasticity of demand (i.e., zero price flexibility, Appendix A), and a 
constant demand over time by setting the growth in household income 
...
(Table 3). Alternative discount rates were half and twice the 5\% value 
used in the baseline simulations. Alternative rates of growth in household 
income were +/-1 standard deviation of the recent mean rate of 3.9\%^[https://www2.gov.bc.ca/assets/gov/british-columbians-our-governments/government-finances/financial-economic-review/financial-economic-review-2020.pdf].
