...
at effort $E_p$ (dependent on numbers-at-age in each area $p$), 
...
unit prices varied according to the covariance estimated when fitting 
the price model in Appendix B to ex-vessel unit price data from
2006 - 2016.
...
series of price data, and recruitment process deviation correlation 
...
## Risks of correlation in practice
...
simulation cross tested harvest strategies that accounted for or ignored 
correlation with the operating model scenarios used to derive those 
strategies under dynamic conditions above. In total, based on
similarity of results, we tested 16 out of 80 potential OM/AM 
combinations, comprising four OM correlation scenarios, ten target 
harvest (two steady-state and eight dynamic optima), and two levels of 
uncertainty in biomass when setting TACs.
...
the **assErr** uncertainty scenario. The assessment error distribution was
derived from assessment errors in previous work [@johnson2021hierarchical], 
where we used closed loop simulations to test a hierarchical surplus 
production assessment model on the DER complex (see section S.2 for
details).

Risk was defined as the difference in outcomes (biomass/rent) between 
target harvest rates under each operating model scenario. Given that 
correlation will have an effect on the distribution of the price and 
recruitment deviations, and therefore more of an effect on 
the variance than the mean or median outcome, we measure risk as the
difference in the 5th percentiles of distributions of biomass and 
net present value. Using a percentile $p$ as a risk metric is known 
as *Value-at-risk-$p$* ($VAR-p$), which is a non-parametric definition 
of risk commonly used in economic analyses [@danielsson2011financial].
