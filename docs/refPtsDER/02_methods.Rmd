...
landings, and adjusted to 2016 values to remove the influence of
inflation [use real CPI deflator]. All ex-vessel prices were specific to 
the bottom trawl fishery except for English Sole in 2016, which was an 
outlier with a trawl ex-vessel price almost three times the 2006 - 2016 
average, and so was replaced with the 2016 ex-vessel price for English 
sole averaged across all available gear types. Fuel costs of fishing data 
were taken from a 2009 economic analysis of the BC trawl fishery 
...
assumed to have a lag-1 auto-correlation in the residuals 
[@francis2014replacing]. Expected age compositions were computed from 
fishery catch-at-age, while expected length compositions were computed 
from vulnerable numbers-at-age by applying an age-length key derived from 
the integrated growth model [@francis2016growth].
...
[@monnahan2017faster; @monnahan2018no], drawing 4 chains of 1000 samples
each. Posterior chains were considered converged when $\widehat{R} < 1.01$ 
and effective sample sizes were greater than 400 (i.e., 100 samples per 
chain) for leading biological paremeters (e.g., $B_0$). Convergence
criteria for non-leading parameters (e.g., process error deviations
and hierarchical deviations) were more relaxed, with $\widehat{R} < 1.05$
and effective sample sizes greater than 360 (i.e., 90 samples per chain).
...
Economic yield was defined as net resource rent, or producer surplus, over 
...
\end{equation}
where $\pi$ is the net resource rent for BC,
...
$E_{p,t}$ were found via stochastic optimisation of  under future 
