...
multi-species fishery system. Second, we detail the bio-economic simulation
...
waters, we focused here on Dover sole, English sole, and Rock sole. Taken 
together, these species comprised a multi-stock complex (DER complex) managed 
...
Island (WCVI), as described in @johnson2021hierarchical. [UPDATE FIGURE
TO USE AREA NAMES AND AGGREGATE STAT AREAS, AS IN 2021] 
...
multi-area population dynamics model. All nine populations of each species in
...
The economic sub-model determined the resource rent from the DER complex
fishery at each annual time step. Resource rent was defined as revenues to
...
Appendix A for details of demand analysis), and $c_{p}$ was the fuel 
cost-per-unit-effort of fishing in area $p$ (or $p'$ for the inner 
summation). The catch was calculated from the Baranov catch equation,
given effort $E_p$ and biomass-at-age $\vec{B}_{s,p}$ 
...
of groundfish catch landed in 2009 [@nelson2011pacific-commerc]. While
constant fuel cost is somewhat unrealistic, given the variation in global
oil prices, it will suffice here. The fuel cost per kilotonne was then 
converted to cost per unit of fishing effort in each area by scaling by 
the ratio of catch to fishing effort from each area in 2009, i.e., 
\begin{equation}
...
function of fishing effort are well-established, standard calculations 
for both single-species fisheries and multi-species fisheries 
...
Single-species $MSY_{SS}$ reference points, in the sense that they ignore
technical interactions, were found by solving for the effort level that 
maximised individual species yield within each area, ignoring any 
by-product/bycatch of other species (Table 1). To incorporate 
...
\end{equation*}
or maximising net present value (NPV) of the DER complex resource
...
of stock biomass over the reference period for stochastic optima (i.e., 
2040 - 2060) to single-species $B_{MSY}$ values, and economic performance 
was measured as the net present value of the DER complex fishery over the 
whole projection. To measure risk, we compared the 5th, 50th (median), 
and 95th percentiles of biomass and NPV distributions. Using a percentile 
$p$ as a risk metric is known as *Value-at-risk-$p$* ($VAR-p$), which is a 
...
sensitivity of stochastic optimisation results (under the rent objective) 
to these uncertainties in seven additional scenarios (Table 3). Three scenarios 
tested the common assumptions about demeand, specifically a flat demand 
relationship, simulated by setting price elasticity of demand to be infinite 
(i.e., zero price flexibility), and a constant demand (over time) by setting 
the growth in household income $\gamma = 0$. The remaining four scenarios 
looked at sensitivites to assumed rates. Alternative discount rates were 
half and twice the 5\% value used in the main simulations. Alternative rates 
of growth in household income were 1 standard deviation up and down from 
the recent mean rate of 3.9\% 
