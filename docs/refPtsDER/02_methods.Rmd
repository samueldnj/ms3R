...
Economic yield is defined as the sum of net resource rent over all areas
\begin{equation}
\pi(\vec{E}) = \sum_{p = 1}^{3} \left( (1 - \psi) \sum_{s} v_{s} \left(\sum_{p} C_{s,p}(E_{p}) \right) \cdot C_{s,p}(E_{p}) \right) - c_{p} \cdot E_{p},
\end{equation}
where $\pi$ is the equilibrium net resource rent for all of BC,
$\vec{E} = \langle E_p \vert p = 1,2,3 \rangle$ is a vector giving trawl
...
effort $E_p$, and $c_{p}$ is the fuel cost-per-unit-effort of fishing in area $p$.

The unit price $v_s$ ($/kg) of each species $s$ followed a downward sloping 
inverse demand curve for total species $s$ catch, with a constant elasticity of 
demand, i.e.,
\begin{equation} 
v_{s}(\sum_{p} C_{s,p}) = v^*_{s} \left( \frac{\sum_p C_{s,p}}{ \sum_p MSY_{SS,s,p}}  \right) ^{1/\lambda_s}
\end{equation}
where $v^*{s}$ was a hypothetical base price of species $s$ associated 
with the sum of single-species MSY, and $\lambda_s < 0$ is the elasticity
of demand [@varian1992microeconomic], both found by fitting to average 
...
and keep prices higher through artificial scarcity, which may result in 
a transfer of benefits (surplus) from consumers to producers 
...
fixed costs is that it is difficult 
...
For illustrative purposes, equilibrium relationships between fishing effort and 
resource rent for individual areas were derived under the joint inverse demand 
curves $v_s$. To incorporate the effect of spatially heterogeneous catch on 
unit price, a grid search was conducted over 100 levels of total coastwide effort 
equally spaced from 0 - 100,000 hours of trawling, assuming that a fixed positive
level of total BC effort would be allocated among areas to maximise rent. For each 
effort grid point, the allocation of effort to each area was computed via 
Newton-Rhapson optimisation, from which area-specific equilibria were derived.
...
last year of the AM (2017 - 2096). Projections were repeated over 150 
replicates, which differed by the random number seed used to draw 
recruitment process deviations and random walk deviations for the unit 
price at $MSY$ of each species in each simulation, i.e., for a projection 
with no correlation
\begin{align*}
...
but also avoided the end effect where the objective function that maximises
catch will start to overfish towards the end of the projection period. 
Loss of consumer and producer surplus was also estimated for each effort 
and correlation scenario by comparing the economic yield 
