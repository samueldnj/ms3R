...
flatfish under the noCorr scenario. Effort (1000s of trawl hours) and 
rent (millons CAD\\$) are calculated by area, while catch ($MSY$, 
$C_{MEY}$, kt), biomass ($B_{MSY}$, $B_{MEY}$, kt), and harvest 
rate ($U_{MSY}$, $U_{MEY}$, \\%) are calculated by species within an area."

discRentTabCap <- "Median and central 90\\% of the distribution of total 
discounted resource rent from the DER complex in the projection period,
over all simulation replicates for the simulated errors and perfect info 
closed loop simulations. Total resource rent is summed over all DER complex 
species and stock areas, and discounted by an annual rate of 5\\%."
```
...
```{r dynEqTables, echo = FALSE, warning = FALSE}
# noCorrEqbriaTab <- makeDynEqbriaTab(  "./data/dynEqbriaTables",
#                                       scenario = "noCorr")
...
perfInfoTab <- read.csv("./data/perfInfoPerfTables/dynEqDiscRentDist.csv",
                        row.names = 1, stringsAsFactors = FALSE)[,-c(1,3:13)]

simErrTab   <- read.csv("./data/simAssErrPerfTables/dynEqDiscRentDist.csv",
                          row.names = 1, stringsAsFactors = FALSE)[,-c(1,3:13)]

scenNames <- c("noCorr","corrRecDevs","corrPriceDevs","corrRecPrice")
mpNames <- c("MSY","MEY","corrRec.MEY","corrRecPr.MEY")
...
perfInfoTab <- perfInfoTab %>% 
                ...
                arrange(  match(MP,mpNames),
                          match(corrScen,scenNames) )

discRentTab <- simErrTab %>% 
                left_join( mpScenGrid, by = "scenMPname") %>%
                dplyr::select( corrScen,
                        MP,
                        cwSimErr = BC ) %>%
                arrange(  match(corrScen,scenNames),
                          match(MP,mpNames) ) %>%
                left_join( perfInfoTab, by = c("MP","corrScen"))

colnames(discRentTab) <- c( "corrScen","HR",
                            "Simulated Errors",
                            "Perfect Info")


csasdown::csas_table( discRentTab[,-1], escape = FALSE,
                      booktabs = TRUE,
                      font_size = 10,
                      format = kable_format,
                      caption = discRentTabCap,
                      align = c("l",rep("c",ncol(discRentTab)-1)) ) %>%
      kable_styling(  latex_options = c("striped", "hold_position","scale_down"),
                      bootstrap_options = c("striped", "hover")) %>%
      add_header_above( header = c( " " = 1,
                                    "Coastwide Discounted Rent ($M)" = 2),
                        bold = TRUE) %>%
      kableExtra::group_rows("No Correlation", 1, 4 ) %>%
      kableExtra::group_rows("Correlated Recruitment Devs", 5, 8 ) %>%
      kableExtra::group_rows("Correlated Price Devs", 9, 12 ) %>%
      kableExtra::group_rows("Correlated Price and Recruitment Devs", 13, 16 )
