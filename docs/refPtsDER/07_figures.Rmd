...
```{r fig2-inverseDemandCurves, echo = FALSE, warning = FALSE, fig.cap = econDashCap, fig.width = 6, out.width = "7in" }
figPath <- file.path("./figure/fig2-inverseDemandCurves.pdf")
if(params$redoPlots)
{
  graphics.off()
  pdf(file = figPath, width = 7, height = 7 )
  plotDemCurves( model = priceModel)
  dev.off()
}
knitr::include_graphics(figPath)
```
...
```{r fig3-DERcomplexCatchYieldCurves, echo = FALSE, warning = FALSE, fig.cap = econYieldCurveCap }
...
if(params$redoPlots | !file.exists(figPath))
{
  graphics.off()
  pdf(file = figPath, width = 8.5, height = 11)
  compareTulipEffort( groupFolder = "omniRuns_noCorr_Apr14",
                      scenarios = "noCorr",
                      rpSim = params$sim,
                      fleets = 1:2,
                      combineEff = TRUE,
...
if(params$redoPlots | !file.exists(figPath))
{
  graphics.off()
  pdf(file = figPath, width = 8.5, height = 11)
  plotDynUmsy_sp( groupFolder = "omniRuns_noCorr_Apr14",
                  mpFilter = "freeEff",
...
if(params$redoPlots | !file.exists(figPath))
{
  graphics.off()
  pdf(file = figPath, width = 8.5, height = 11)
  plotBioDist( groupFolders = c("simAssErrors_Apr14") )
  dev.off()
}
knitr::include_graphics(figPath)
```
...
```{r fig8-simEnvelope, echo = FALSE, warning = FALSE, fig.cap = imperfectInfoSimEnvCap }
figPath <- file.path("./figure/fig8-simEnvelope.pdf")
if(params$redoPlots | !file.exists(figPath))
{
  .loadSim(1,groupFolder=  "simAssErrors_Apr14")
  graphics.off()
...
if(params$redoPlots | !file.exists(figPath))
{
  graphics.off()
  pdf(file = figPath, width = 8.5, height = 11)
  compareTulipRent( groupFolder = "simAssErrors_Apr14",
                    scenarios = "noCorr",
