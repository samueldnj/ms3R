...
Very few fisheries are truly single-species fisheries. With
the possible exception of a small number of high-seas pelagic 
fisheries, most fishing effort encounters multiple species

@howell2021combining
@mace2001new
@walters2005possible
@hollowed2000multispecies
@kinzey2009multispecies
@block1995single
...
factors are often ignored when setting fishery management objectives. On
the contrary, objectives in multi-species fisheries are typicall based on
single-species biological reference points, such as maximum sustainable 
yield (MSY) or some proxy [@collie2001biological], or on extensions of 
biological reference points such as maximum economic yield (MEY)
[@hoshino2018estimating]. Economic yield (also known as resource rent or 
producer surplus, as opposed to absolute catch), which is one part of 
'triple bottom line' fisheries management (the other two being biological 
conservation and social outcomes), is increasinly recommended as a 
decision axis in fisheries management frameworks [@benson2018options; 
...
MSY of the prey species may be impossible in the presence of the 
predator, because the predation mortality may already be above the level 
associated with MSY in the absence of that predator [@may1979management].
In such cases, managers may want to increase prey species yield by choosing
reference points in line with objectives to lower predator abundance 
and predation mortality in favour of higher fishery yield of the prey 
species. In multi-species fisheries where competitive 
...
Despite the existence of several multi-species and ecosystem models that 
...
[@kasperski2015optimal]. Such species are said to have technical 
interactions, where the species caught incidentally is referred to 
as *byproduct* when the catch can be landed, and *bycatch* 
when it must be discarded for legal reasons or when it is unmarketable. 
If byproduct/bycatch species are included in an ecologically interacting 
multi-species complex for which reference points are derived, but the 
technical interactions are ignored, then incidental catch will often 
...
easier to catch species have their catch limits fulfilled faster, thereby
limiting fishing opportunities for other species that may have higher catch
limits or landed value [@baudron2015adverse].
...
species specific fishing mortality to a common index of aggregate 
fishing effort or, where set-by-set effort and catch composition data
is available, further broken down by target species to account for 
different technical interactions depending on the target species 
[@punt2011calculating; @punt2020multispecies]. Commercial catchability 
scalars, therefore, represent the relative strength of technical 
interactions within a multi-species system that managers and harvesters 
must contend with [@baudron2015adverse]. Multi-species effort-yield 
relationships are then straightforward to derive [@pikitch1987use], 
but calculation of optima may require simulation if the system is 
especially complex [@punt2011calculating]. 

In some jurisdictions, management objectives aiming to maximise the
economic yield from multi-species fisheries require additional
bio-economic modelling to derive net economic benefit, or resource 
rent [@kasperski2015optimal; @hoshino2018estimating]. 
Multi-species reference points with technical interactions are 
commonly used as the basis for bio-economic modelling, while
ecological interactions are more often ignored. Costs of fishing and 
landed values are readily added to multi-species effort-yield 
relationships with technical interactions as outlined above, and estimation 
of net present values are straightforward if variable cost functions 
are simple (e.g., linear in fishing effort). More sophistication is 
required as the complexity of the economic model increases, for example, 
when including individual vessel bheaviour such as entry/exit from the 
...
@punt2020multispecies]. Such cases usually require simulation approaches
to optimization.
...
@smith2012new], because variation in relative abundance of species 
among areas may imply different management objectives or different 
harvester preferences [@sanchirico1999bioeconomics; @sanchirico2001dynamics; 
@sanchirico2005optimal; @sanchirico2007sustainable]. If spatial patches and 
fishing effort are defined at a larger spatio-temporal scale, then variation
in relative abundance may imply different relative catchabilities, and 
therefore different multispecies effort-yield relationships. Spatially 
heterogeneous variable fishing costs then combine with spatially heterogeneous 
effort-yield relationships to affect resource rents in each patch. 
Despite differences in the optimal efforts and yields between patches, 
there are several similarities to the non-spatial case. For example, 
maximum economic yield calculations within each patch are identical to 
single-area calculations when effort allocation is independent among areas 
and there no fish dispersal between areas [@sanchirico2005optimal; 
@hoshino2018estimating]. When effort is unregulated (i.e., an open access 
system), analytical models of spatially heterogeneous single-species 
fisheries show that effort will increase until net benefits are dissipated 
as in the single-area fishery case [@sanchirico2001dynamics; 
@clark2010mathematical]. When effort is limited by regulation (e.g., total 
number of licenses or a tax on landings), then the optimal effort allocation 
varies depending on the regulations in each spatial unit and any underlying 
dispersal process [@sanchirico2005optimal]. Under any regulation scheme, 
it is unlikely that aggregate management approaches will be economically 
efficient if the underlying spatial dynamics or structure are ignored
[@smith2009economics; @goethel2017accounting].
...
(usually logistic models) are a popular choice for bio-economic 
...
yield calculations apply an average revenue model, where a species' unit 
price is the same for all levels of catch (i.e., no change in demand). 
...
tend to expend less effort to artificially create scarcity, increasing 
their profit margins through higher landed values and lower costs of 
fishing [@pascoe2018offsetting]. Similarly, cost functions are also 
usually simplified in fisheries literature. While it is difficult to 
capture all fishing costs, some cost functions are simply linear with 
fishing effort, excluding capital costs, social costs 
...
In this paper, we asked how optimal harvest rates (i.e. fishery 
objectives) for a spatially heterogeneous, multi-species flatfish fishery 
differed between static and dynamic equilibria when maximimising catch or 
resource rent. We used closed loop simulation to simulate a three species 
complex of right-eyed flounders (*Pleuronectidae Spp.*) - Dover sole 
(*Microstomus pacificus*), English sole (*Parophrys vetulus*), and 
(Southern) Rock sole (*Lepidopsetta bilineata*) - fished in three 
spatial strata by a common trawl fishery off the coast of British Columbia 
(BC), Canada. Steady-state multi-species maximum yield reference points 
incorporated technical interactions of the three species to relate 
species- and area-specific yield curves to a common trawl effort 
[@pikitch1987use], and multi-species economic yield calculations 
incorporated spatially heterogeneous costs of fishing and downward sloping 
demand curves with a constant price elasticity of demand. 
Solving for dynamic equilibria in the presence of price and recruitment
deviations required stochastic optimisation of fishing under two objective
functions, which either maximised catch or the net present value of the 
DER complex fishery. Risk was measured by comparing distributions 
of biomass, catch, and net present value when optimal harvest 
rates were used to set TACs in closed loop simulation, under both perfect 
and imperfect knowledge about stock biomass.
