...
(Table 4). While $E_{MEY}$ in QCS was higher than in WCVI by a similar 
proportion to the corresponding $E_{MSY}$ values, the resource rent was 
about 40\% higher in the WCVI area. Higher WCVI rent is largely caused 
by the higher Dover sole biomass relative to English and Rock sole 
biomasses in the WCVI, along with the higher price elasticity of demand 
for Dover sole making prices more stable as catch is increased, compensating 
for higher simulated fuel costs of fishing in the WCVI area.
...
Stochasticity had no significant effects on dynamic approximations
to $U_{MSY}$. Median $U_{MSY}^*$ harvest rates were practically identical 
to steady-state $U_{MSY}$ harvest rates, differing by a tenth of a 
percentage point, or around 0.5\% of the steady state optimum (Table 6). 
In contrast, harvest rates under the NPV maximising objective 
functions had much lower variability and $U_{MEY}^*$ rates were consistently 
higher than the steady-state $U_{MEY}$ values by around 4\% - 7\% (Table 6).
As expected, price correlation had a noticeable effect on harvest
rates that maximised NPV, with median harvest rates in the absence of 
price correlation consistently higher than median harvest rates 
in the presence of price correlation, by about 5\% in HSHS, and 2\% in
QCS and WCVI. Despite higher median values and lower variability, all 
harvest rate distributions under NPV maximising objective functions 
contained the steady state solution (Figure 10).
...
resource rent columns, Tables 4 and 7), despite almost identical efforts 
...
We presented results from the closed loop simulation under perfect
information and simulated assessment errors for three target harvest 
rates. Those harvest rates were $U_{MSY,MS}$ and $U_{MEY}$ estimates
at static equilibrium, and the dynamic approximation $U_{MEY}&*$. We 
do not simulate catch using the dynamic approximations 
...
statistically significant, we present results based on the two 
