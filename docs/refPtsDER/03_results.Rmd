...
the Hecate Strait assemblage survey for all three species (Figure SX); poor 
...
Fits to commercial trawl age compositions were also acceptable.
...
[Reference to supplemental material for referece points from
base run without PED?]
...
4 and 6). [**EFFORT PLOT WILL HELP EXPLAIN** - probably caused by 
finite time-horizon as well ramping down of harvest in HSHG] 
Although dynamic $U_{MEY}^*$ median values were consistently
higher than steady state equilibria, the distribution of dynamic harvest 
rates under NPV maximising objective functions contained the steady state 
solution (Figure 7). 
...
higher CPUE), which led to higher resource rents despite lower unit prices
implied by the downward sloping demand curve.
    - **Compare time series of annual rents over projection period**
...
- Unclear whether this is a development effect or an artifact
    ...
    to derive target HRs. How can we check?
...
On the other hand, imperfect information appeared to reduce rents under 
rent maximising harvest rates. Median NPVs and $VAR-95\%$ values 
were $1.7M - $2M ($100,000 - $125,000 per year) lower when TACs 
