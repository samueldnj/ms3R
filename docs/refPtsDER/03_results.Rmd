...
dist after integrating over 100 possible futures.**] 
...
under each correlation scenario, namely the two steady state solutions
and one dynamic approximation to $U_{MEY}$. Given that median harvest 
rates for dynamic approximations to $U_{MSY,MS}$ were almost identical 
to the steady state solution, we do not present results from closed 
loop simulations that applied dynamic $U_{MSY,MS}$ harvest rates to 
DER complex stocks. Although differences in steady-state and median 
dynamic $U_{MEY}$ harvest rates were not statistically significant, 
we present results based on applying both the steady state $U_{MEY}$ 
and its dynamic approximation under the full price and recruitment
correlation scenario, because fishery management decisions are often 
made based on mean or median values, with variability sometimes ignored 
or used to justify additional precautionary controls. However, the
four dynamic approximations to $U_{MEY}$ differed by at most one
percentage point, so one approximation will give sufficiently
