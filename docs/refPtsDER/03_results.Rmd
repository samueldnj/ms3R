...
Similar to above, MEY was highest in the HSHG area at the lowest effort,
...
the WCVI area. Higher WCVI rent is largely caused by the higher Dover sole 
biomass relative to English and Rock sole biomasses in the WCVI, along 
with the higher price elasticity of demand for Dover sole making prices
more stable as catch is increased, compensating for higher simulated 
fuel costs of fishing in the WCVI area.
...
$\langle E_1, E_2, E_3 \rangle = \langle 0.251E, 0.419E, 0.330E \rangle$.
This allocation was very close to the vector 
$\vec{\rho} = \langle 0.256, 0.416, 0.328 \rangle$ of relative 
inverse average commercial catchabilty scalars within an area, defined by
\begin{align*}
\rho_p &= \frac{\mu_{q^{(F)},p}^{(inv)}}{\sum_p \mu_{q^{(F)},p}^{(inv)}}, \\
\mu_{q^F,p}^{inv} & = \frac{1}{3} \sum_s \frac{1}{q^{(F)}_{s,p}}, \\
\end{align*}
...
[reference to supplement - look at effect of PED by removing it].
The optimal allocation was also similar to the relative proportions of 
$E_{MSY,MS,p}$, which were allocated as $\langle 0.27,0.385,0.345 \rangle$.
...
objective functions that maximised total catch, median harvest rates 
were almost identical to steady-state optimal harvest rates (Table 4),
while their distributions showed high variability. Despite lower 
variability of harvest rates under rent maximisation, distributions
still intersected with steady-state $U_{MEY}$ values, but median
harvest rates were consistently higher than steady-state optimal harvest 
rates by between 7\% and 40\%.
...
the shape of distributions more than the scale. Median harvest rates for
catch maximisation were insensitive to the correlation scenario, differing 
by around half a percent across all scenarios (compare Table 5 and Tables 
S1 - S3). In contrast, median rent maximising harvest rates were somewhat 
sensitive to correlation in price, increasing by 4\% - 5\% in HSHG and 
around 2\% in QCS and WCVI when price correlation was simulated. 
[**concrete differences in percentiles**]

[*Correlation effects were most 
noticeable (i.e., visually) under recruitment correlation, and less 
noticeable under price correlation (Figure 10). 
...
Median annual (undiscounted - nominal?) resource rents under both 
objective functions were higher than the steady-state solution (compare 
resource rents columns, Tables 4 and 5), despite almost identical efforts 
and harvest rates under the catch maximising objective function. When 
maximising catch under the stochastic optimisation, median rent was over 
three times the steady state estimate in HSHG, around two times in QCS, 
and 9\% lower in WCVI. The rent maximising objective function also produced
higher resource rents than steady state solutions, extracting 88\% higher 
rents in HSHG, 50\% higher in QCS, and 14\% higher in WCVI. Higher resource 
rents in general are caused by the [**ability of prices to climb higher 
through price variation than under steady state, which relies on price 
elasticity of demand alone to increase surplus. Further, there is a maximum 
price of 200\% of the historically observed max, but a minimum of 50\% of the 
...
deterministic runs will show whether this is true.**] - also, is there
an effect of maximising the total discounted rent here? Maybe try
a deterministic run without discounting to see.
...
the HSHG area in the presence of price correlation. 


## Targeting optimal harvest rates in practice

We present results from the closed loop simulation under perfect
information and simulated assessment errors for four harvest rates
under each correlation scenario. The four harvest rates we present
are the steady state $U_{MSY,MS}$ and $U_{MEY}$ estimates, and 
the two dynamic approximations to $U_{MEY}$ estimated when recruitment
correlation was simulated (i.e., **allCorr** and **recCorr** 
scenarios). We do not present dynamic approximations to the $U_{MSY,MS}$
harvest rates, given that median harvest rates derived from the catch 
maximising objective function were almost identical to the steady 
state solution. However, while the differences in steady-state and median 
dynamic $U_{MEY}$ harvest rates were not statistically significant, we 
present results based on the three distinct estimates because fishery 
management decisions are often made based on mean or median stock 
assessment outputs, with variability sometimes ignored or used to 
justify additional precautionary controls, and differences of 5\% -
40\% in harvest rates would create significant differences in management
outcomes. 
