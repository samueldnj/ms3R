...
were almost identical to steady-state optimal harvest rates (Table 6),
while their distributions showed very high variability. Despite lower 
variability of harvest rates that maximised NPV, distributions
still intersected with steady-state $U_{MEY}$ values, while median
harvest rates were consistently higher than steady-state $U_{MEY}$
values by between 7\% and 40\% (compare Tables 4 and 6).
...
elasticity of demand alone to increase resource rent. Further, there was 
a maximum price of 200\% of the historically observed max, but a minimum 
of 50\% of the historically observed min price, which induced asymmetry 
in the price dist after integrating over 100 possible futures. Testing using
...
scenarios). We do not simulate catch using the dynamic approximations 
to catch maximising harvest rates $U_{MSY,MS}$, given that median harvest 
rates derived from the catch maximising objective function were almost 
identical to the steady state solution. Although the differences 
in steady-state and median dynamic $U_{MEY}$ harvest rates were not 
statistically significant, we present results based on the three 
distinct estimates because fishery management decisions are often 
made based on mean or median stock assessment outputs, with 
variability sometimes ignored or used to justify additional 
precautionary controls, and increases of 7\% - 40\% in harvest rates 
would create significant differences in management outcomes. 
